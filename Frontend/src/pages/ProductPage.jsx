...
const ProductPage = () => {
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get('https://kryzenbiotec-1-0sgh.onrender.com/product/my-prod', {
    ...
    const handleAddProduct = async (newProduct) => {
        try {
            await axios.post('https://kryzenbiotec-1-0sgh.onrender.com/product/create', newProduct, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('userToken')}` }
            });
            fetchProducts();
        } catch (error) {
    ...
    const handleEditProduct = async (updatedProduct) => {
        try {
            await axios.put(`https://kryzenbiotec-1-0sgh.onrender.com/product/edit/${updatedProduct.id}`, updatedProduct, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('userToken')}` }
            });
            fetchProducts();
        } catch (error) {
    ...
    const handleDeleteProduct = async (id) => {
        try {
            await axios.delete(`https://kryzenbiotec-1-0sgh.onrender.com/product/delete/${id}`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('userToken')}` }
            });
            toast({
